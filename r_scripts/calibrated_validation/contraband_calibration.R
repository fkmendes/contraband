...
longest.20.branches <- tr$edge.length %in% tail(sort(tr$edge.length), n=20)
tr$edge.length[longest.20.branches] = tr$edge.length[longest.20.branches] * .1 
...
names(true.param.df) <- c("sigmasq", "mu")
