...
job.prefix <- args[7] # e.g., "BMMVNShift" or "BMPruneShift"
time.needed <- args[8] # e.g., "00:15:00"
template.name <- args[9]
tree.type <- args[10]
xmlfolder.path <- paste0(cal.validation.folder, job.prefix, "OneTraitOneRate_", tree.type, "_xmls/")
xml.file.prefix <- args[11]
shell.scripts.path <- paste0(cal.validation.folder, job.prefix, "OneTraitOneRate_", tree.type, "_shellscripts/")
