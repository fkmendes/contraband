...
set.seed(234)
dat <- mvSIM(tr, model="BMM", param=list(sigma=c(0.01, 0.02), theta=0.0))
rownames(dat) <- c("sp1", "sp2", "sp3")

res <- mvBM(tr, dat, model="BMM")

res$LogLik # -4.673609
res$sigma # 0.05057867 3.360241 note that in Java I have to invert them because the rates need to be identifiable (the second always > than first)
res$theta # -1.191236
...
lambda <- rexp(1, rate=80) # lambda from exponential (mean = 1/80)
mu <- rexp(1, rate=100) # mu from exponential (mean = 1/100)
tr <- sim.bd.taxa.age(50, 1, lambda, mu, age=100, mrca=TRUE)[[1]];
...
set.seed(1)
sigmas <- list(black=0.01, red=0.5, green=0.1)
dat <- mvSIM(tr, nsim=1, model="BMM", param=list(ntraits=1, sigma=sigmas, theta=0.0))
res <- mvBM(tr, dat, model="BMM")

res$sigma # 0.006850499, 0.08316839, 0.2736696
res$theta # -0.05011816
res$LogLik # -80.18669
