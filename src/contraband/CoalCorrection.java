...
//		storedCorrectedPhyloTMat = new double[nSpp][nSpp];
		...
		scaleX = new double[nNodes];
		parentP = new int[nNodes];
	...
	private int fillNLineageDistInPlace(Node aNode) {
		if (aNode.isLeaf()) {
			 ...
			 */
			int parent = aNode.getParent().getNr();
			if (parentP[aNode.getNr()] != parent) {
				parentP[aNode.getNr()] = parent;
				return -1;
			}
            return 1;
		...
		// if internal node or root
            ...
            double scale = -aNode.getLength() / thisNodePopSize;
            if (parentP[nodeIdx] != parent ||
            	leftX[nodeIdx] != maxNLineagesLeft || 
            	rightX[nodeIdx] != maxNLineagesRight || 
            	scaleX[nodeIdx] != scale) {
            	isDirty = true;
            }
            parentP[nodeIdx] = parent;
            leftX[nodeIdx] = maxNLineagesLeft; 
            rightX[nodeIdx] = maxNLineagesRight; 
            scaleX[nodeIdx] = scale;
						...
						if (aNode.isRoot()) {
							...
							for (int kLineages=1; kLineages <= nLineages; ++kLineages) {
								...
								nodel[kLineages-1] += probOfNLineages *
										getHeledGij(nLineages, kLineages, scale); /*aNode.getLength(), thisNodePopSize);*/
							}
						}
					}
				}
					...
					for (int nLineages=1; nLineages <= maxNLineages; ++nLineages) {
						...
						if (aNode.isRoot()) {
							...
							for (int kLineages = 1; kLineages <= nLineages; ++kLineages) {
								...
								nodel[kLineages - 1] += probOfNLineages * getHeledGij(nLineages, kLineages, scale); /*
